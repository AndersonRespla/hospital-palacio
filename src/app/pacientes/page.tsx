...
export default function PacientesPage() {
  ...
  const handleCreate = async (data: PacienteVirtual) => {
    try {
  ...
  const handleEdit = async (data: PacienteVirtual) => {
    try {
      const res = await fetch(`/api/pacientes/${data.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          <table className="min-w-full table-auto border-collapse bg-white rounded-lg shadow-sm overflow-hidden mt-4">
            <thead>
              <tr>
                ...
                <th className="border-b border-gray-200 bg-gray-50 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nome</th>
                <th className="border-b border-gray-200 bg-gray-50 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Doença</th>
                <th className="border-b border-gray-200 bg-gray-50 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Custo/mês</th>
            ...
            <tbody>
              {items.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 transition-colors duration-150">
                  <td className="border-b border-gray-200 px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{p.id}</td>
                  <td className="border-b border-gray-200 px-6 py-4 whitespace-nowrap text-sm text-gray-700">{p.nomeFicticio}</td>
                  <td className="border-b border-gray-200 px-6 py-4 whitespace-nowrap text-sm text-gray-700">{p.siglaDoenca}</td>
                  <td className="border-b border-gray-200 px-6 py-4 whitespace-nowrap text-sm text-gray-700">R$ {p.custoPorCamaMes.toLocaleString()}</td>
                  <td className="border-b border-gray-200 px-6 py-4 whitespace-nowrap text-sm space-x-3">
                    <Link href={`/pacientes/${p.id}`} className="text-blue-600 hover:text-blue-800 font-medium transition-colors duration-150 inline-flex items-center">
  ...
  return (
    <>
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)}>
        <h2 className="text-xl font-bold mb-4 text-gray-800">Novo Paciente</h2>
        <FormPaciente onSubmit={handleCreate} />
      </Modal>
      
      <Modal isOpen={showEdit} onClose={() => setShowEdit(false)}>
        <h2 className="text-xl font-bold mb-4 text-gray-800">Editar Paciente</h2>
        {selected && <FormPaciente initialData={selected} onSubmit={handleEdit} />}
      </Modal>
    </>
  );
}
